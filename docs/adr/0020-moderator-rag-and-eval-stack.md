...
- **RAGAS is not adopted as a separate framework**: DeepEval subsumes its metrics. The RAGAS surface is DeepEval's NATIVE re-implementation (faithfulness, contextual precision / recall / relevancy), gated in `test_rag_metrics.py`. *Amended 2026-06-26:* DeepEval 4.x moved the literal `RagasMetric` wrapper behind the external `ragas` package, which pins `langchain_community.chat_models.vertexai` — a path removed in langchain 1.x, the moderator's stack — so it cannot be installed without breaking production langchain. The wrapper eval (`test_ragas_via_deepeval_wrapper`) is kept as a breadcrumb but SKIPS unless `ragas` is importable; the native metrics carry the technique. This *reinforces* the "no redundant parallel dependency" call rather than reversing it.
