 ...
 * human-readable doc, the README hero (kicker + heat-grid SVG) and teaser blocks, and the SVGs
 * themselves. Derived-only (render-claims discipline): every cell, count, and pixel comes from
 * the artifact; nothing is hand-typed.
 *
 *   pnpm matrix:render           # write docs/detection-matrix.md + docs/assets/*.svg + check README blocks are pasted
...
export const HERO_END = '<!-- matrix-hero:end -->'

function renderHeroBlock(): string {
  return `${HERO_START}
...
<img alt="The detection matrix as a heat grid: ${counts.caught} filled cells (catches) and ${counts.missed} hollow cells (recorded misses) across three tiers" src="docs/assets/detection-matrix-light.svg">
</picture></a>
${HERO_END}`
}
...
export const TEASER_START =
  '<!-- matrix-teaser:start (generated by `pnpm matrix:render`; do not edit) -->'
export const TEASER_END = '<!-- matrix-teaser:end -->'

function renderTeaserBlock(): string {
...
function checkReadmeFragment(
  ...
  if (a === -1 || b === -1) return `README ${name} block markers absent`
  if (readme.slice(a, b + end.length) !== render()) return `README ${name} block is stale`
  return null
}
...
function main(): void {
  ...
  if (process.argv.includes('--check')) {
    ...
    for (const [name, start, end, render] of [
      ['matrix-hero', HERO_START, HERO_END, renderHeroBlock],
      ['matrix-teaser', TEASER_START, TEASER_END, renderTeaserBlock],
    ] as const) {
      const p = checkReadmeFragment(name, start, end, render)
    ...
    process.stdout.write('✓ matrix doc, SVGs, and README blocks match a fresh render\n')
    process.exit(0)
  }

  if (process.argv.includes('--readme')) {
    process.stdout.write(`${renderHeroBlock()}\n\n${renderTeaserBlock()}\n`)
    return
  }
  ...
  for (const [name, start, end, render] of [
    ...
    const p = checkReadmeFragment(name, start, end, render)
    if (p) process.stdout.write(`note: ${p} — print with pnpm matrix:render --readme and paste\n`)
  }
}
