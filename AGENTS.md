...
4. **For state machine changes:** edit the XState machine in `packages/contracts/src/machines/`, update the conformance test, update any MBT-generated tests.
...
- `packages/contracts/src/test-results-schema.ts`: the schema for `test-results/summary.json` is frozen.
...
delta set: CI adds `prove:adversarial`, the web component census, the headless Storybook
...
4. `test-results/summary.json` is produced and validates against its frozen schema. The reusable `_envelope.yml` terminal job fans every lane's evidence partial (from the same run) into one envelope; a key/cluster lane that never ran is deferred; a lane that DID run and folded no runner is fatal (`QAROOM_RAN_LANES`, derived from the partials the envelope actually downloaded).
