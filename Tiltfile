...
# Langfuse (the LLM-trace UI spike) is a 7-pod ClickHouse/MinIO/Redis/Postgres stack that CI never
# runs and most dev loops don't need. OFF by default; opt in with `tilt up -- --langfuse`.
#
# Calling config.parse() takes over Tilt's CLI arg handling, so positional resource names
# (`tilt up web` to iterate on one service) stop selecting a subset unless we consume them
# ourselves. Declare a positional `to-run` list and re-enable just those resources when given.
config.define_string_list('to-run', args=True)
config.define_bool('langfuse')
_cfg = config.parse()
enable_langfuse = _cfg.get('langfuse', False)
_to_run = _cfg.get('to-run', [])
if _to_run:
    config.set_enabled_resources(_to_run)
...
# gc-dedup-cronjob.yaml dropped: the CronJob is now generated by the shared chart per service
# (deploy/<svc>/values.yaml `gc.enabled`), so the standalone manifest was objectless.
_observability = [
...
if enable_langfuse:
    _observability.append('deploy/observability/langfuse.yaml')
k8s_yaml(_observability)
...
# Python moderator: its own pgvector Postgres + NATS (it subscribes to post.created). When Langfuse
# is enabled it also waits on langfuse-web so the ON-BOOT seed (prompt + golden dataset + annotation
# queue) runs after the API is up (one-shot; a too-early boot would skip it). With Langfuse OFF the
# seed no-ops — trace export and the seed are both best-effort, so the moderator boots fine without it.
_moderator_deps = ['moderator-agent-pg', 'qaroom-nats', 'content']
if enable_langfuse:
    _moderator_deps.append('qaroom-langfuse-web')
k8s_resource('moderator-agent', port_forwards='8086:8086', resource_deps=_moderator_deps, labels=['services'])
...
# Langfuse v3 LLM-trace UI (spike) — OFF by default (opt in: `tilt up -- --langfuse`). Backends
# first, then the bucket-init Job, then web + worker. web on :3001. UI at http://langfuse.localhost.
if enable_langfuse:
...
_langfuse_note = " · Langfuse :3001 (langfuse.localhost)" if enable_langfuse else " · Langfuse OFF (enable: tilt up -- --langfuse)"
print("QARoom on k3d — web :8085 · gateway :8080 · webhooks :8087 · moderator :8086 · Jaeger :16686 · Grafana :3000 · Prometheus :9090 · NATS :4222 · Tracetest :11633 · Microcks :8888" + _langfuse_note)
