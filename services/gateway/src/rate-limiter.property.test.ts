...
describe('rate limiter (property)', () => {
  test.prop([fc.integer({ min: 1, max: 30 }), fc.integer({ min: 0, max: 80 })])(
    'never allows more than capacity requests before any refill',
    (capacity, n) => {
  ...
  test.prop([fc.integer({ min: 1, max: 8 }), fc.integer({ min: 1, max: 10 })])(
    'a denied request always reports a positive retry-after when the bucket refills over time',
