...
import { setupWebhooksTest } from './harness'
 ...
 */
describe('webhooks-service serves every operation it declares', () => {
  let ctx: Awaited<ReturnType<typeof setupWebhooksTest>>
  afterEach(async () => {
    await ctx.close()
  })

  it('every operation in the registry is registered on the app (not just documented)', async () => {
    ctx = await setupWebhooksTest()
