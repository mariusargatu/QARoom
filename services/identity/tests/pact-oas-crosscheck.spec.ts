 ...
 * identity-service is consistent with identity-service's PUBLISHED OpenAPI contract. Catches the gap Pact
...
const PACT = resolve(import.meta.dirname, '..', '..', 'gateway', 'pacts', 'gateway-identity.json')

describe('gateway pact ↔ identity OpenAPI cross-check', () => {
  it.each(
    pactInteractions(PACT),
  )('interaction "$description" matches the identity OpenAPI operation it exercises', async (interaction) => {
