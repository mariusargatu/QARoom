...
- **Clients:** `src/api/client.ts` (gateway; idempotency keys from the injected `IdGenerator` — a
  ULID, globally unique across reloads, NOT a per-load counter, which would reset on refresh and
  replay a stale idempotency response. No `Date.now()`/`crypto.randomUUID()`: the determinism lint
  applies to browser `.ts` too), `src/ws/client.ts`.
...
- **Component tests** (`.browser.test.tsx`, ADR-0027 — supersedes Playwright CT): render with
  `vitest-browser-react`'s `await render(...)` and assert directly. THREE of them additionally drive
  the component through the same Screenplay Tasks/Questions the E2E suite uses, via
...
- **Visual regression** (ADR-0027 §3): Vitest `expect.element(locator).toMatchScreenshot()` (pixel
  ...
  canonical baseline IS committed (`*-chromium-linux.png`); only the non-Linux and
  `-actual`/`-diff` variants are gitignored.
...
- **Coverage**: all V8, no Istanbul merge — but THREE configs write three directories
  (`coverage-node/`, `coverage-component/`, `coverage/`). The number that gates is
  `pnpm coverage` at the repo root, which runs `vitest.coverage.config.ts` (node + component as
  one pass) against a floor in `scripts/coverage.ts`.
...
`vitest-browser-react` 2.2 / Playwright 1.61 (E2E only) / Tailwind 4.3 / React 19 / Vite 8.1 /
`@vitejs/plugin-react` 6.0. Three test surfaces, all Vitest: the `unit` project (`pnpm test`, node),
