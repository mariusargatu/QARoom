 ...
 * validated domain objects — a promise `client.request.test.ts` enforces by asserting a
 * contract-violating 2xx REJECTS on every method (the response-shaped tests in `client.test.ts` all
 * pass without any parsing at all).
 *
 * Most of the REST plane is unauthenticated by design; the two exceptions carry a bearer — the
 * WS-ticket mint, and the events poll behind ADR-0025's edge auth.
 */
export interface ApiClient {
  ...
  // events
  /**
   * The Commitment-11 polling fallback. `accessToken` is REQUIRED against a real gateway: ADR-0025
   * put this route behind edge auth (`verifyToken.verify(req.headers.authorization)` before the
   * membership check) and the spec attaches `authRequired401`. Optional only so a caller with no
   * session can still be typed; such a poll will 401.
   */
  listEvents(communityId: string, after: number, accessToken?: string): Promise<EventPage>
  // webhooks
...
/**
 * Every path parameter goes through this. Ids reach the client straight from `useParams`, i.e. from
 * the URL bar: interpolated raw, a crafted `../../something` walks the request off its endpoint to
 * an attacker-chosen same-origin path — carrying a valid Idempotency-Key on a mutation. Encoding
 * keeps a hostile id a single path SEGMENT, where the gateway's `CommunityId.parse` rejects it.
 */
const seg = (value: string) => encodeURIComponent(value)

const comm = (id: string) => `/api/communities/${seg(id)}`
...
export function createApiClient(baseUrl: string, ids: IdGenerator): ApiClient {
  ...
  return {
    ...
    getUser: (userId) => http.get(`/api/users/${seg(userId)}`, (r) => User.parse(r)),
    ...
    getPost: (postId) => http.get(`/api/posts/${seg(postId)}`, (r) => Post.parse(r)),
    createPost: (communityId, body) =>
      http.post(`${comm(communityId)}/posts`, body, (r) => Post.parse(r)),
    castVote: (postId, body) =>
      http.post(`/api/posts/${seg(postId)}/votes`, body, (r) => CastVoteResponse.parse(r)),
    // donations
    ...
    resolveFlag: (communityId, flagKey) =>
      http.get(`${comm(communityId)}/flags/${seg(flagKey)}`, (r) => FlagResolution.parse(r)),
    listFlags: (communityId) => http.get(`${comm(communityId)}/flags`, (r) => FlagList.parse(r)),
    advanceRollout: (communityId, flagKey, event) =>
      http.post(`${comm(communityId)}/flags/${seg(flagKey)}/rollout`, { event }, (r) =>
        FlagResolution.parse(r),
      ),
    // events
    listEvents: (communityId, after, accessToken) =>
      http.get(
        `${comm(communityId)}/events?after=${after}`,
        (r) => EventPage.parse(r),
    ...
    getWebhook: (communityId, subscriptionId) =>
      http.get(`${comm(communityId)}/webhook-subscriptions/${seg(subscriptionId)}`, (r) =>
    ...
    deleteWebhook: (communityId, subscriptionId) =>
      http.del(`${comm(communityId)}/webhook-subscriptions/${seg(subscriptionId)}`),
    pauseWebhook: (communityId, subscriptionId) =>
      http.post(
        `${comm(communityId)}/webhook-subscriptions/${seg(subscriptionId)}/pause`,
        {},
        (r) => WebhookSubscription.parse(r),
      ),
    resumeWebhook: (communityId, subscriptionId) =>
      http.post(
        `${comm(communityId)}/webhook-subscriptions/${seg(subscriptionId)}/resume`,
        {},
        (r) => WebhookSubscription.parse(r),
      ),
    listWebhookDeliveries: (communityId, subscriptionId) =>
      http.get(
        `${comm(communityId)}/webhook-subscriptions/${seg(subscriptionId)}/deliveries`,
        (r) => WebhookDeliveryList.parse(r),
      ),
    // moderation
    ...
    getModerationDecision: (communityId, decisionId) =>
      http.get(`${comm(communityId)}/moderation-decisions/${seg(decisionId)}`, (r) =>
