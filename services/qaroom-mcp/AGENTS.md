...
- The HTTP `/mcp` endpoint is **unauthenticated by design** (read-first surface — the same edge-auth-deferred-to-M13 rationale as the gateway REST plane, single-sourced in [ADR-0022](../../docs/adr/0022-gateway-fronts-identity-and-moderation-for-the-web-edge.md) / [`ARCHITECTURE.md` §7](../../ARCHITECTURE.md#7-what-this-architecture-deliberately-omits-and-why)): protect it at the network layer; do not add a mutating tool without auth + an ADR amendment. The conventions oracle caps snippet size (`maxLength` on the tool schema + an in-oracle guard): keep that bound when editing it.
